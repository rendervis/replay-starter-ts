  ...
  loop({ props, state, device }) {
    ...
    if (lastPipe.x < 140) {
      pipes = [...pipes, newPipe(device)]
        ...
        .filter((pipe) => pipe.x > -(device.size.width + device.size.widthMargin + pipeWidth));
    }
...
function didHitPipe(birdY: number, size: DeviceSize, pipes: PipeT[]) {
  ...
  for (const pipe of pipes) {
    if (pipe.x > birdX + birdWidth / 2 + pipeWidth / 2 || pipe.x < birdX - birdWidth / 2 - pipeWidth / 2) {
      // bird isn't at pipe
      continue;
    }
    const { yUpperTop, yUpperBottom, yLowerTop, yLowerBottom } = getPipeYPositions(size, pipe.gapY);
    const topPipeRect = {
      x: pipe.x,
    ...
    if (birdPoints.some((point) => pointInRect(point, topPipeRect) || pointInRect(point, bottomPipeRect))) {
      // Bird hit a pipe!
      return true;
    }
  }
...
function pointInRect(point: { x: number; y: number }, rect: { x: number; y: number; height: number; width: number }) {
  return (
